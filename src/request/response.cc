...
Response Response::stock_reply(Response::status_type status)
{
  ...
  std::pair<std::string, std::string> a ("Content-Length", std::to_string(rep.content.size()));
  rep.headers_.insert(a);
  std::pair<std::string, std::string> b ("Content-Type", "text/html");
  rep.headers_.insert(b);
  
  for (auto x : rep.headers_)
  {
